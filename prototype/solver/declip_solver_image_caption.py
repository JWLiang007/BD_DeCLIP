...
from prototype.utils.misc import visual_img
...
class ClsSolver(BaseSolver):
    ...
    def find_last_checkpoint(self):
        checkpoint_dir = os.path.join('checkpoints',self.config.saver.save_prefix)
        ckpt_list = os.listdir(checkpoint_dir)
        if 'ckpt.pth.tar' in ckpt_list:
            return f'{checkpoint_dir}/ckpt.pth.tar'
        elif len(ckpt_list) == 0:
            return None
        num = [int(ckpt.split('.')[0][5:]) for ckpt in ckpt_list]
        num.sort()
        last_checkpoint_path = f'{checkpoint_dir}/ckpt_' + str(num[-1])+'.pth.tar'
    ...
    def train(self):
        ...
        # for i, batch in enumerate(self.train_data['loader']):
            ...
            if 'CLSA' not in self.config.data.train.transforms.type:
                ...
                nad_loss = torch.zeros([]).cuda()
                if self.tea_image_encode is not None:
                    with torch.no_grad():
                        images_1, images_2 = torch.split(batch.images, [3,3], dim=1)
                        self.tea_image_encode(images_1,prefix = 'img_1')
                        self.tea_image_encode(images_2,prefix ='img_2')

                    for img_i in range(1,3):
                        for feat_i in range(1,5):
                            nad_loss += self.at( eval(f'self.model.module.visual.img_{img_i}_feat_{feat_i}') , eval(f'self.tea_image_encode.module.img_{img_i}_feat_{feat_i}'))
                    # if 'adv_images' in batch:
                    #     batch.adv_images = batch.adv_images[:,:3,:,:].cuda()
                    #     with torch.no_grad():
                    #         self.tea_image_encode(batch.adv_images,prefix = 'adv')
                    #     self.model.module.visual(batch.adv_images,prefix = 'adv')  
                    #     for feat_i in range(1,5):  
                    #         adv_feat_loss = F.mse_loss(eval(f'self.model.module.visual.adv_feat_{feat_i}') , eval(f'self.tea_image_encode.module.adv_feat_{feat_i}'))     
                    #         nad_loss += adv_feat_loss   
            ...
            reduced_nad_loss = nad_loss.clone()
            ...
            if math.isnan(self.meters.losses.avg) or math.isnan(reduced_loss) or reduced_loss > self.meters.losses.avg + 10:
                ...
                self.meters.nad_losses.reduce_update(reduced_nad_loss)
            ...
            if curr_step % self.config.saver.print_freq == 0 and self.dist.rank == 0:
                ...
                self.tb_logger.add_scalar(
                    'nad_loss_train', self.meters.nad_losses.avg, curr_step)
                self.tb_logger.add_scalar(
                ...
                log_msg = f'Iter: [{curr_step}/{total_step}]\t' \
                    ...
                    f'Nad Loss {self.meters.nad_losses.val:.4f} ({self.meters.nad_losses.avg:.4f})\t' \
            ...
            if curr_step > 0 and curr_step % self.config.saver.val_freq == 0:
                ...
                while True:
                    try:
                        self._state = torch.load(ckpt_name, 'cpu')
                        load_state_model(self.model, self._state['model'])
                        break
                    except Exception:
                        continue
                link.barrier()
